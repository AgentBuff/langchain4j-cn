...
When an output guardrail returns a retry outcome, the LLM is called again with the original prompt and chat history.

#### Reprompt {#reprompt}

When an output guardrail returns a reprompt outcome, an additional prompt message is appended before the LLM is called again.
